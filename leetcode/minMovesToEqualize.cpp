...
public:
  ...
  int increment_nums(vector<int>& nums) {
    if (nums.size() < 2) return 0;
    int minDelta = INT_MAX;
    for (int i = 0; i < nums.size(); ++i) {
      int diff = nums[i+1] - nums[i];
      if (diff > 0 && diff < minDelta) minDelta = diff;
    }
    //cout << "Found min delta = " << minDelta << "\n";
  ...
  int minMovesInternal(vector<int>& nums, int res) {
    if (allEqual(nums)) return res;
    //cout << "not equal yet \n";
    //printNums(nums);
    sort(nums.begin(), nums.end());
    res += increment_nums(nums);
    //res++;
    printNums(nums);
    return minMovesInternal(nums, res);
  }

  /*
  int minMoves(vector<int>& nums) {
    if (!nums.size()) return 0;
    return minMovesInternal(nums, 0);
  }
  */
